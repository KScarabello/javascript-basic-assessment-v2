...
function tripleTheFun(param){
  if(Number(param)){
    return param * 3;
  } else if (parseInt(param) === isNaN(param)){
    return parseInt(param)*3;
  } else {
...
function getPropertyValue(objectName, propName){
  
 
 var value = objectName[propName];
...
function whatIsItOutside(tempNum, humNum, cloudyNum){
  if (tempNum > 85 && humNum > 45){
    return "I'm all sweat";
  } else if (tempNum < 40 && cloudyNum > 60){
    return "I have icicles";
  } else if (tempNum > 85 && humNum < 45 && cloudyNum < 20){
    return "I'm literally in the desert";
  } else if (tempNum > 90 || humNum > 55 || cloudyNum >65){
